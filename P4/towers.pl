...
% Maxim 'MaxNumTowers' towerPos() poden ser certes --> nomes es consideren les posicions que son de pobles --> es crearien moltes clausules!
% ==>
...
writeClauses(MaxNumTowers):-
    eachVillageMaxOneTower,             % cada poble, maxim una torre
    eachSignVillageOneTower,            % cada poble significant, exactament una torre
    respectMaxTowers(MaxNumTowers),                   % at most max villages with tower
    eachVillageIsWatched,               % cada poble, te almenys una posicio on hi ha una tower en la seva columna o fila
    eachTowerInsideVillage,             % cap torre pot estar fora d'un poble
    relateTowerPosWithTowerVill,        % relacio entre les dues sat variables
    true,!.                             % this way you can comment out ANY previous line of writeClauses
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachVillageMaxOneTower :-
    village(V), findall(towerPos(I,J), posVillage(V,I,J), L), atMost(1,L), fail.
eachVillageMaxOneTower.

eachSignVillageOneTower :- 
    significantVillage(V), 
    writeOneClause([towerVillage(V)]),
    fail.
eachSignVillageOneTower.

respectMaxTowers(MaxNumTowers) :- 
    findall(towerVillage(V), village(V), L), 
    upperLimitTowers(M), 
    Max is min(M, MaxNumTowers),
    atMost(Max,L), 
    fail.
respectMaxTowers(_).

eachVillageIsWatched :-
    village(V), findall(towerPos(I,J), posWatchesVillage(V,I,J), L), atLeast(1,L), fail.
eachVillageIsWatched.

eachTowerInsideVillage :-
    position(I,J),
...
% towerVillage(V) <--> towerPos(1,2) v towerPos(1,3) v ... v towerPos(2,5)
%
%   towerVillage(V) --> towerPos(1,2) v towerPos(1,3) v ... v towerPos(2,5)
%   -towerVillage(V) v towerPos(1,2) v towerPos(1,3) v ... v towerPos(2,5)
%
%   towerPos(1,2) v towerPos(1,3) v ... v towerPos(2,5) -> towerVillage(V)
%   -(towerPos(1,2) v towerPos(1,3) v ... v towerPos(2,5)) v towerVillage(V)
%   -towerPos(1,2) & -towerPos(1,3) & ... & -towerPos(2,5) v towerVillage(V)
relateTowerPosWithTowerVill :-
    village(V),
    findall(towerPos(I,J), posVillage(V,I,J), L),
    expressOr(towerVillage(V), L),
    fail.
relateTowerPosWithTowerVill.
...
costOfThisSolution(M,Cost):- findall(V, member(towerVillage(V),M), L), length(L,Cost).
