...
writeClauses(MaxConsecutiveHours) :-
    eachGangsterHourOneTask, % un gangster en una hora nomes pot fer una tasca
    noDiffTaskOnConsecHours, % un gangster no pot fer dues tasques diferents en hores consecutives
    respectAvailability,     % respectar disponibilitat dels gangsters
    taskNeeds,
    respectMaxConsecHours(MaxConsecutiveHours),
    relateVariables,
    true,!.
writeClauses(_) :- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachGangsterHourOneTask :-
    gangster(G), hour(H), findall(does(G,T,H), task(T), L), atMost(1,L), fail.
eachGangsterHourOneTask.

noDiffTaskOnConsecHours :- 
    gangster(G),
    task(T1), task(T2), T1 \= T2,
    hour(H1), hour(H2), H2 is H1+1,
    writeOneClause([-does(G,T1,H1), -does(G,T2,H2)]),
    fail.
noDiffTaskOnConsecHours.

respectAvailability :-
    blocked(G,H), writeOneClause([-gangsterWorksAtHour(G,H)]), fail.
respectAvailability.

taskNeeds :-
    needed(T,H,N),
    findall(does(G,T,H), gangster(G), L), exactly(N,L),
    fail.
taskNeeds.

respectMaxConsecHours(MaxConsecutiveHours) :-
    gangster(G), 
    hour(S), hour(E), E is S + MaxConsecutiveHours,
    findall(gangsterWorksAtHour(G,H), between(S,E,H), L),
    negateAll(L,NegL),
    atLeast(1,NegL),
    fail.
respectMaxConsecHours(_).

relateVariables :- 
    gangster(G), hour(H),
    findall(does(G,T,H), task(T), L),
    expressOr(gangsterWorksAtHour(G,H), L),
    fail.
relateVariables.
...
costOfThisSolution(M,Cost) :-
    between(0,71,N), Cost is 72 - N, % cost va disminuint desde el maxim (72) i quan trobi un que es cert pararà
    gangster(G),
    HiniMax is 72-Cost,
    between(1, HiniMax, Hini), Hfi is Hini+Cost-1,
    findall(gangsterWorksAtHour(G, H), (member(gangsterWorksAtHour(G, H), M), between(Hini, Hfi, H)), L), 
    length(L, Cost).
