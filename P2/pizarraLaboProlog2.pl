...
cifras(L,N):-
    ...
    N is E,
    write(E), nl, fail. % fail --> provoca bcktracking per reconsiderar expressions

% per ordenar els subcjts per tamany
cifras(L,N):-
	length(L,Lsize).
	between(1,Lsize,Long),
    subcjto(L,S),         % S = [4,8,7,100]
	length(L,Long),
    permutation(S,P),     % P = [4,100,7,8]
    expresion(P,E),       % E = 4 * (100-7) + 8 
...
% el operador de corte "!": las clausulas de posibles ptos de backtracking se eliminan y no se reconsideran
...
union( [],     L,  L ).
union( [X|L1], L2, U     ):-     member(X,L2),!, union( L1, L2, U ). % --> no repetir
union( [X|L1], L2, [X|U] ):-                     union( L1, L2, U ).

% cuando vea que N es negativo verá el ! y no reconsiderará las otras opciones y entrará en el 2o positive
positive(N) :- N<0, !, fail.
positive(_).
