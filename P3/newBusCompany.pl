...
writeClauses :- 
    ...
    eachTrajectOnceMaxDay,
    eachTrajectMinDaysWeek,
    departsFromTheLastDest,
    respectMaxKmOnConsecDays,
    true,!.
writeClauses :- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactlyOneTripPerBusAndDay :- 
    % Per cada dia, un bus nomes fa un trajecte
    bus(B), day(D), findall(bdcc(B,D,C1,C2), trip(C1-C2), L), exactly(1,L), fail.
exactlyOneTripPerBusAndDay.

eachTrajectOnceMaxDay :- 
    % Cada trajecte es fa maxim un cop al dia
    trip(C1-C2), day(D), findall(bdcc(B,D,C1,C2), bus(B), L), atMost(1,L), fail.
eachTrajectOnceMaxDay. 

eachTrajectMinDaysWeek :-
    % Cada trajecte s'ha de fer un minim de dies a la setmana (frequency)
    trip(C1-C2), findall(bdcc(B,D,C1,C2), (bus(B),day(D)), L), frequency(C1-C2, Min), atLeast(Min,L), fail.
eachTrajectMinDaysWeek.

departsFromTheLastDest :-
    % Per cada trajecte d'un bus, el desti del primer trajecte es l'origen del segon si son dies consecutius
    % --> no pot haver-hi dos viatges en dos dies consecutius tal que el desti del 1er sigui igual al origen del segon
    bus(B), consecutiveDays(D1, D2),    
    trip(C1-C2), trip(C3-C4), C2 \= C3,
    negateAll([bdcc(B,D1,C1,C2), bdcc(B,D2,C3,C4)], N),
    writeOneClause(N),
    fail.
departsFromTheLastDest.

respectMaxKmOnConsecDays :-
    % Un bus no pot superar el maxim de kms en dos dies consecutius
    bus(B),
    consecutiveDays(D1,D2),
    tooLongDist(C1-C2-C3),
    negateAll([bdcc(B,D1,C1,C2), bdcc(B,D2,C2,C3)], N),
    writeOneClause(N), 
    fail.
respectMaxKmOnConsecDays.
