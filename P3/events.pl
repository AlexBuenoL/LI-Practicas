...
writeClauses :-  
    ...
    eachEventOnlyInAllowedDays,
    eachEventOnlyByAllowedModerators,

    eachDayMaxEvents,
    eachModeratorMaxDays,

    relateEDandEMwithMD, % em(E,M) ^ ed(E,D) --> md(M,D)
    true,!.
writeClauses :- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachEventExactlyOneDay:- 
    event(E),
    findall(ed(E,D), day(D), L),
    exactly(1, L), fail.
eachEventExactlyOneDay.

eachEventExactlyOneModerator :-
    event(E),
    findall(em(E,M), moderator(M), L),
    exactly(1,L),
    fail.
eachEventExactlyOneModerator.

eachEventOnlyInAllowedDays :-
    % Donat un event, nomes es pot donar en dies dels possibles
    event(E), day(D),
    eventDays(E,PosDays),
    \+member(D,PosDays),
    writeOneClause([-ed(E,D)]),
    fail.
eachEventOnlyInAllowedDays.

eachEventOnlyByAllowedModerators :- 
    % Donat un event, nomes el modera un moderador permes
    event(E), moderator(M),
    eventModerators(E, PosMods),
    \+member(M,PosMods),
    writeOneClause([-em(E,M)]),
    fail.
eachEventOnlyByAllowedModerators.

eachDayMaxEvents :- 
    % En un dia donat, el nombre d'events es menor o igual al maxim d'events
    day(D),
...
eachModeratorMaxDays :-
    % El nombre de dias que un moderador pot moderar un event no supera el maxim
    % Com relaciones el moderador amb els dies?? --> No es pot amb les satVariables que tenim --> necessitem crear un altra
    moderator(M),
