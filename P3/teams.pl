...
%% numWorkers(20). --> 20*5 variables --> nomes 20 seran certes
...
incompatibleWorkers(W1,W2) :- maxScore(M), workerScore(W1,S1), workerScore(W2,S2), W1 \= W2, S1+S2 > M.
...
writeClauses :-
    ...
    %% 1 Cada treballador, nomes esta a un equip
    eachWorkerExactlyOneTeam,

    %% 2 En cada equipo numWorkers >= minSize and <= maxSize
    sizesMinMaxRespected,

    %% 3 Para cada par de trabajadores, son compatibles
    limitScore,
...
eachWorkerExactlyOneTeam :- 
        worker(W), findall(wt(W,T), team(T), Lits), exactly(1, Lits), fail.
eachWorkerExactlyOneTeam.

sizesMinMaxRespected :- 
        team(T), findall(wt(W,T), worker(W), Lits), 
        minSize(Min), maxSize(Max), atLeast(Min, Lits), atMost(Max, Lits), fail.
sizesMinMaxRespected.
