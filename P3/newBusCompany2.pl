...
satVariable( bdc(B,D,C) ) :- bus(B), day(D), trip(_-C).
...
eachTrajectOnceMaxDay :- 
    % Cada trajecte es fa maxim un cop en un dia
...
departsFromTheLastDest :-
    ...
    % bdcc(B,D2,C2,C3) -> bdc(B,D1,C2) == -bdcc(B,D2,C2,C3) v bdc(B,D1,C2)
    bus(B), consecutiveDays(D1, D2),    
    trip(C2-C3),
    writeOneClause([-bdcc(B,D2,C2,C3), bdc(B,D1,C2)]),
    fail.
departsFromTheLastDest.

relateBDCandBDCC :-
    bus(B), day(D), trip(_-C2),
    writeOneClause( [-bdcc(B, D, _, C2), bdc(B, D, C2)]),
    fail.
relateBDCandBDCC.
